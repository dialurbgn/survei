...
class Master_menu extends MX_Controller {
		...
		function get_data(){
			...
			foreach ($list as $rows) {
				...
				$viewdata = '<div class="menu-item px-3"><a target="_blank" class="dropdown-item d-flex align-items-center gap-2" href="'.base_url().$rows['module'].'"><i class="fa fa-eye text-info mt-1"></i> View</a></div> ';
		...
		public function actiondata($id){
			...
			if($query_column){
				if($id != '0'){
					$exclude = array('module','color','history_id','status_id','created','modified','createdid','modifiedid','id','active','slug');
					$this->ortyd->access_check_update($this->module);
					if($query_column){
					...
					if(count($data) > 1){
						...
						if ($this->db->trans_status() === FALSE){
							...
							if($update){
								...
								$new_module = $this->input->post('module');
								$build = $this->input->post('build');
								$this->saveModule($new_module, $build);
		...
		function saveModule($new_module, $build = 1){
			if($build == 1){
				$module_name = 'Master_template';
				$module_target_name = ucfirst($new_module);
				$source =  './application/modules/'.strtolower($module_name);
				$dest =  './application/modules/'.strtolower($module_target_name);
				$filenya = $this->ortyd->recursive_copy($module_name,$module_target_name,$source, $dest);
				if($filenya == 1){
					$this->ortyd->setMenu($new_module);
				}
				return $filenya;
			}else{
				$this->ortyd->setMenu($new_module);
