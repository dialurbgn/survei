...
class Users_groups extends MX_Controller {
		...
		//CONFIG VARIABLE
		private $urlparent = 'users_groups'; //NAME TABLE 
		...
		private $exclude = array('level','color','history_id','status_id','created','modified','createdid','modifiedid','id','active','slug');
		...
		function get_data(){
			...
			foreach ($list as $rows) {
				...
				if($this->ortyd->access_check_update_data($this->module)){
					...
					$editdata = '<div class="menu-item px-3"><a class="dropdown-item d-flex align-items-center gap-2" href="'.base_url().$this->urlparent.'/editdata/'.$identity_id.'"><i class="fa fa-edit"></i> Edit & Set Permision</a></div> ';
		...
		public function getMenuChild(){
			...
			$show='';
			$menu_access = [];
			$this->db->select('users_groups_access.id, users_groups_access.view, users_groups_access.insert, users_groups_access.update, users_groups_access.delete');
			$this->db->where_in("users_groups_access.menu_id",$parent_id);
			$this->db->where("users_groups_access.gid",$gid);
			$queryheadmenu = $this->db->get('users_groups_access');
			...
			if($queryheadmenu){
				foreach($queryheadmenu as $rowsmenu){
					$menu_access = array(
							"id" => $rowsmenu->id,
							"view" => $rowsmenu->view,
							"insert" => $rowsmenu->insert,
							"update" => $rowsmenu->update,
							"delete" => $rowsmenu->delete,
					);	
				}
			}
			...
			$menu_kiri = [];
			$menu_gid = [];
			$this->db->select('master_menu.id, master_menu.name, master_menu.show, users_groups_access.menu_id as menunya');
			$this->db->where_in("master_menu.parent_id",$parent_id);
			$this->db->where_in("users_groups_access.gid",$gid);
			...
			if($queryheadmenu){
				foreach($queryheadmenu as $rowsmenu){
					...
					array_push($menu_kiri, array(
							"id" => $rowsmenu->id,
							"name" => $rowsmenu->name.' '.$show,
						)
					);	
				}
			}else{
			...
			$result = array("csrf_hash" => $this->security->get_csrf_hash(),"message" => "success", "menu_kiri" => $menu_kiri, "menu_kanan" => $menu_kanan, "menu_access" => $menu_access );
			echo json_encode($result);
		...
		public function getPermissionChild(){
			...
			$result = array("csrf_hash" => $this->security->get_csrf_hash(),"message" => "success", "menu_access" => $menu_access );
			echo json_encode($result);

		}
		
		
		function saveAccess($gid){
			$result = array("csrf_hash" => $this->security->get_csrf_hash(),"message" => "error");
			if($gid != '' && $gid != null){
				...
				if($menu_id != null && $menu_id != ''){
					...
					if(count($menu_id) > 0){
						...
						//$this->db->where('master_menu.show', 1);
						$this->db->where('users_groups_access.gid', $gid);
						$this->db->join('master_menu','master_menu.id = users_groups_access.menu_id',"left"); 
						$this->db->group_by('master_menu.id');
						$query = $this->db->get('users_groups_access');
						$query = $query->result_object();
						if($query){
							foreach($query as $rows){
								array_push($list_id, $rows->id);
							}
						}else{
							$this->db->select('master_menu.id');
							if($tipe == 1){
						...
						$this->db->where_in('users_groups_access.gid', $gid);
						$this->db->where_in('users_groups_access.menu_id', $list_id);
						$update = $this->db->update('users_groups_access', $datadetail);
						
						if($update){
							foreach( $menu_id as $key => $n ) {

								if($menu_id[$key] != '' && $menu_id[$key] != '0' && $menu_id[$key] != 0){
									$this->db->where('users_groups_access.gid', $gid);
									$this->db->where('users_groups_access.menu_id', $menu_id[$key]);
									$query = $this->db->get('users_groups_access');
									$query = $query->result_object();
									if(!$query){
										...
										$insert = $this->db->insert('users_groups_access', $datadetail);	
										$insert_id = $this->db->insert_id();
										...
										$this->db->where('users_groups_access.id', $query[0]->id);
										$update = $this->db->update('users_groups_access', $datadetail);
		...
		function saveAccessPermission($gid){
			if($gid != '' && $gid != null){
				
				//$tenggat_sph = $this->input->post('tenggat_sph');
				$menu_id = $this->input->post('menu_id')[0] ?? 0;
				$view = $this->input->post('view') ?? 0;
				$insert = $this->input->post('insert') ?? 0;
				$update = $this->input->post('update') ?? 0;
