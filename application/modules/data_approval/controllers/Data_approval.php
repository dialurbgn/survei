...
class Data_approval extends MX_Controller {

		//CONFIG VARIABLE
		private $urlparent = 'data_approval'; //NAME TABLE 
		...
		private $exclude = array('type_id','color','history_id','status_id','created','modified','createdid','modifiedid','id','active','slug');
		private $exclude_table = array('type_id','color','history_id','status_id','created','modified','createdid','modifiedid','id','active','slug');
		//END CONFIG VARIABLE
		...
		public function getDataRoleRequired(){
			...
			$this->db->select('data_approval_role.*, users_groups.name as role_name, master_status_option.name as is_ttd_name');
			$this->db->where('data_approval.id',$approval_id);
			$this->db->where('data_approval.active',1);
			$this->db->where('data_approval_role.active',1);
			$this->db->join('data_approval_role','data_approval_role.approval_id = data_approval.id','left');
			$this->db->join('users_groups','users_groups.id = data_approval_role.role_id','left');
			$this->db->join('master_status_option','master_status_option.id = data_approval_role.is_ttd','left');
			$this->db->order_by('data_approval_role.level','ASC');
			$query = $this->db->get('data_approval');
			$query = $query->result_object();
			if($query){
		...
		function saveRoleApproval($approval_id,$keydata = 0){
			if($approval_id != '' && $approval_id != null){
				...
				$tbl_ttd_id = $this->input->post('approval_role_ttd_id')[$keydata] ?? 1;
				$tbl_level = $this->input->post('approval_role_level')[$keydata] ?? null;
				$tbl_name = $this->input->post('approval_role_name')[$keydata] ?? null;
				...
				if($tbl_role_id != null && $tbl_role_id != ''){
					...
					if(count($tbl_role_id) > 0){
						...
						$this->db->where('data_approval_role.approval_id', $approval_id);
						$update = $this->db->update('data_approval_role', $datadetail);
						if($update){
							foreach( $tbl_role_id as $key => $n ) {
								...
								if($tbl_approval_role_id[$key] != '' && $tbl_approval_role_id[$key] != '0' && $tbl_approval_role_id[$key] != 0){
									$this->db->where('data_approval_role.id', $tbl_approval_role_id[$key]);
									$query = $this->db->get('data_approval_role');
									$query = $query->result_object();
									if(!$query){
										...
										$string = $slugnya;
										$slug = $this->ortyd->sanitize($string,'data_approval_role');
										$datadetail = array_merge($datadetail,
											array('slug' 	=> $slug)
										);
												
										$insert = $this->db->insert('data_approval_role', $datadetail);	
										$insert_id = $this->db->insert_id();
										...
										$this->db->where('data_approval_role.id', $query[0]->id);
										$update = $this->db->update('data_approval_role', $datadetail);
