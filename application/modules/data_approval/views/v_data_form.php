	...
	$.post('<?php echo base_url($headurl.'/getDataRoleRequired'); ?>',{
			...
			if(data != 'null'){
				...
				if(obj.status == 'success'){
					...
					'<div class="div-detail-table"><span style="font-size: 16px;font-family: sora-font-regular;">Role</span></div>'+
							...
							"drawCallback": function( settings ) {
								...
								$.each(json, function(i, item) {
									...
									setTimeout(function(){
										...
										select2option(nomorrole[number],'approval_role_ttd_id_'+number+'_'+nomorrole[number],'Option','master_status_option','id','name',null,null);
									}, 1000);
...
function addRowRole(approval_data_id = null, number = null, i = null, item = null){
	...
	table_rolepo[number].row.add([
			...
			'<input name="approval_role_name['+approval_data_id+'][]" id="approval_role_name_'+number+'_'+nomorrole[number]+'" class="form-control form-control-sm" type="text" value="" required placeholder="Isi name"  />',
			'<input name="approval_role_level['+approval_data_id+'][]" id="approval_role_level_'+number+'_'+nomorrole[number]+'" class="form-control form-control-sm" type="number" value="" required placeholder="Isi Level"  />',
			'<select name="approval_role_ttd_id['+approval_data_id+'][]" placeholder="Isi TTD"  class="form-control form-control-sm" id="approval_role_ttd_id_'+number+'_'+nomorrole[number]+'" required></select>',
	...
	if(item == null){
		...
		$('#approval_role_name_'+number+'_'+nomorrole[number]).val(item.name);
		$('#approval_role_level_'+number+'_'+nomorrole[number]).val(item.level);
		
		var newOptionTTD = new Option(item.is_ttd_name, item.is_ttd, true, true);
        $('#approval_role_ttd_id_'+number+'_'+nomorrole[number]).append(newOptionTTD).trigger('change');
