...
class Translate_table_select extends MX_Controller {

		//CONFIG VARIABLE
		private $urlparent = 'translate_table_select'; //NAME TABLE 
		...
		public function getDataOption(){
			...
			$this->db->where('table_select_id',$data_id);
			$this->db->where('active',1);
			$query = $this->db->get('translate_table_select_option');
			$query = $query->result_object();
			if($query){
		...
		function saveOption($data_id){
			if($data_id != '' && $data_id != null){
	
				$tbl_id = $this->input->post('tbl_id');
				$option_value = $this->input->post('tbl_option_name');
				...
				if($option_value != null && $option_value != ''){
					...
					if(count($option_value) > 0){
						...
						$this->db->where('translate_table_select_option.table_select_id', $data_id);
						$update = $this->db->update('translate_table_select_option', $datadetail);
						
						if($update){
							foreach( $option_value as $key => $n ) {
								
								$slugnya = $option_value[$key];
								
								if($tbl_id[$key] != '' && $tbl_id[$key] != '0' && $tbl_id[$key] != 0){
									$this->db->where('translate_table_select_option.id', $tbl_id[$key]);
									$query = $this->db->get('translate_table_select_option');
									$query = $query->result_object();
									if(!$query){
									
										$datadetail = array(
											'table_select_id' 		=> $data_id,
										...
										$string = $slugnya;
										$slug = $this->ortyd->sanitize($string,'translate_table_select_option');
										$datadetail = array_merge($datadetail,
											array('slug' 	=> $slug)
										);
												
										$insert = $this->db->insert('translate_table_select_option', $datadetail);	
										$insert_id = $this->db->insert_id();
										...
										$this->db->where('translate_table_select_option.id', $query[0]->id);
										$update = $this->db->update('translate_table_select_option', $datadetail);
