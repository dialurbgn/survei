...
class Data_article extends MX_Controller {

		//CONFIG VARIABLE
		private $urlparent = 'data_article'; //NAME TABLE 
		...
		private $exclude = array('is_frontend','color','history_id','status_id','created','modified','createdid','modifiedid','id','active','slug');
		private $exclude_table = array('is_frontend','color','history_id','status_id','created','modified','createdid','modifiedid','id','active','slug');
		//END CONFIG VARIABLE
		...
		public function __construct()
		{
			...
			$this->actionurl_draft = $this->urlparent.'/actiondata_draft';
		...
		function get_data(){
			...
			foreach ($list as $rows) {
				...
				$jenis_slug = $this->ortyd->select2_getname($rows['jenis_id'],'master_article_jenis','id','slug');
				if($rows['is_publish'] == 1){
					$viewdata = '<div class="menu-item px-3"><a target="_blank" class="dropdown-item d-flex align-items-center gap-2" href="'.base_url($jenis_slug).'/'.$identity_id.'"><i class="fa fa-eye text-primary mt-1"></i> View On Web</a></div> ';
				}else{
					$viewdata = '<div class="menu-item px-3"><a target="_blank" class="dropdown-item d-flex align-items-center gap-2" href="'.base_url($jenis_slug).'/'.$identity_id.'?type=preview"><i class="fa fa-eye text-primary mt-1"></i> Preview On Web</a></div> ';
				}
		...
		public function createdata()
		{
			...
			$data['action_draft'] = base_url().$this->actionurl_draft.'/'.$data['id'];
				...
				if($id != '0'){
					...
					if($query_column){
						foreach($query_column as $rows_column){
							...
							}elseif($rows_column["name"] == 'content'){
								$dataisi = $this->input->post($rows_column["name"],false);
								if($dataisi == ''){
									$dataisi = null;
								}
								$data_array = array($rows_column["name"] => $dataisi);
					...
					if($query_column){
						...
						$slug = $this->ortyd->sanitizenormal($string,$this->tabledb);
						$data = array_merge($data,
							array('slug' 	=> $slug)
						);
						//END ADDITIONAL
		...
		public function actiondata_draft($id){
				...
				if($id != '0'){
					...
					if(count($data) > 1){
						...
						if ($this->db->trans_status() === FALSE){
							...
							if($update){
								$this->saveEvidence($id, $this->urlparent);
								$slug = $this->ortyd->select2_getname($id,'data_article','id','slug');
								$jenis_id = $this->ortyd->select2_getname($id,'data_article','id','jenis_id');
								$jenis_slug = $this->ortyd->select2_getname($jenis_id,'master_article_jenis','id','slug');
								if($this->input->post('is_publish', true) == 1){
									$linknya = base_url().$jenis_slug.'/'.$slug;
								}else{
									$linknya = base_url().$jenis_slug.'/'.$slug.'?type=preview';
								}
								
								$result = array("csrf_hash" => $this->security->get_csrf_hash(),"status" => "success",'slug_refresh' => '','preview_url' => $linknya);
								echo json_encode($result);
							}else{
								$result = array("csrf_hash" => $this->security->get_csrf_hash(),"status" => "error",'slug_refresh' => '');
								echo json_encode($result);
							}
						}
					}else{
					...
					if(count($data) > 1){
						...
						if ($this->db->trans_status() === FALSE){
							...
							if($insert){
								$identity = $this->input->post($this->field,true);
								$this->saveEvidence($insert_id, $this->urlparent);
								$result = array("csrf_hash" => $this->security->get_csrf_hash(),"status" => "success",'slug_refresh' => base_url().'data_page/editdata/'.$slug);
								echo json_encode($result);
							}else{
								$result = array("csrf_hash" => $this->security->get_csrf_hash(),"status" => "error",'slug_refresh' => '');
								echo json_encode($result);
