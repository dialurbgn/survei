...
class Master_setting extends MX_Controller {

		//CONFIG VARIABLE
		private $urlparent = 'master_setting'; //NAME TABLE 
		private $identity_id = 'slug'; //IDENTITY TABLE
		private $field = 'slug'; // IDENTITY FROM NAME FOR GET ID
		private $slug_indentity = 'meta_id'; //NAME FIELD 
			...
			foreach ($list as $rows) {
				...
				if($query_column){
					...
					foreach($query_column as $rowsdata){
						$table_references = null;
						$table_references = $this->ortyd->get_table_reference($table,$rowsdata['name']);
						if($rowsdata['name'] == 'meta_value' && $rows['meta_id'] == 'company_map'){
							$variable = 'MAP';
							$row[] = $variable;
						}elseif($table_references != null){
				...
				if($id != '0'){
					...
					if($query_column){
						foreach($query_column as $rows_column){
							...
							}elseif($rows_column["name"] == 'meta_value'){
								if($this->input->post('meta_id',true) == 'company_map'){
