...
class Data_page extends MX_Controller {

		//CONFIG VARIABLE
		private $urlparent = 'data_page'; //NAME TABLE 
		...
		function get_data(){
			...
			foreach ($list as $rows) {
				...
				if($rows['is_publish'] == 1){
					if($rows['jenis_id'] == 2){
						$viewdata = '<div class="menu-item px-3"><a target="_blank" class="dropdown-item d-flex align-items-center gap-2" href="'.base_url().'"><i class="fa fa-eye text-primary mt-1"></i> View On Web</a></div> ';
					}else{
						$viewdata = '<div class="menu-item px-3"><a target="_blank" class="dropdown-item d-flex align-items-center gap-2" href="'.base_url().$identity_id.'"><i class="fa fa-eye text-primary mt-1"></i> View On Web</a></div> ';
					}
					
				}else{
					$viewdata = '<div class="menu-item px-3"><a target="_blank" class="dropdown-item d-flex align-items-center gap-2" href="'.base_url().$identity_id.'?type=preview"><i class="fa fa-eye text-primary mt-1"></i> Preview On Web</a></div> ';
				}
		...
		public function createdata()
		{
			...
			$data['action_draft'] = base_url().$this->actionurl_draft.'/0';
				...
				if($id != '0'){
					...
					if(count($data) > 1){
						...
						if ($this->db->trans_status() === FALSE){
							...
							if($update){
								$this->saveEvidence($id, $this->urlparent);
								$slug = $this->ortyd->select2_getname($id,'data_page','id','slug');
								$jenis_id = $this->ortyd->select2_getname($id,'data_page','id','jenis_id');
								$jenis_slug = $this->ortyd->select2_getname($jenis_id,'master_page_jenis','id','slug');
								if($this->input->post('is_publish', true) == 1 && $jenis_id == 1){
									$linknya = base_url().$slug;
								}elseif($this->input->post('is_publish', true) == 1 && $jenis_id == 2){
									$linknya = base_url();
								}else{
									$linknya = $linknya = base_url().$slug.'?type=preview';
								}
