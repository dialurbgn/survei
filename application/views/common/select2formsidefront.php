		...
		<div class="<?php echo $form_group_class; ?>">
			<div class="<?php echo $row_class; ?>">
				<div class="<?php echo $input_col; ?>">
					...
					<select class="form-control text-3 h-auto py-2 <?php echo $is_popup ? 'select2-popup' : ''; ?> <?php if($rows_column['is_nullable'] == 'NO'){echo ' required ';} ?>" name="<?php echo $rows_column['name']; ?>" id="<?php echo $select_id; ?>" <?php if($rows_column['is_nullable'] == 'NO'){echo ' required';} ?> <?php echo $disable; ?>  placeholder="<?php echo 'Input '.$label_name_text; ?>">
						<?php if($table == 'm_set_wil_administratif') { ?>
							<option value="<?php echo ${$rows_column['name']}; ?>"><?php echo $this->ortyd->select2_getname(${$rows_column['name']},$table,'wil_id',$selecttablename); ?></option>
						<?php }elseif($selecttablename != 'name') { ?>
